...
function uilib.createToggleButton(parent, buttonText, buttonSize, buttonPosition, toggleColor, offColor, Function, Setting)
    ...
    local function updateButtonState()
        if isOn then
            tween:Create(button, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {TextColor3 = toggleColor or Color3.fromRGB(255, 255, 255)}):Play()
        else
            tween:Create(button, TweenInfo.new(0.5, Enum.EasingStyle.Quint), {TextColor3 = offColor or Color3.fromRGB(255, 255, 0)}):Play()
        end
    end
